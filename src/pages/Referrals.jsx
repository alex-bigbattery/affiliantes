...
const ORDER_STATUSES = ['all', 'open', 'estimated', 'unpaid', 'paid', 'pending', 'rejected']

const WP_STATUSES = ['all', 'unpaid', 'paid', 'pending', 'rejected']



const VIEWS = [

  { id: 'wp', label: 'All WP referrals', hint: 'Full AffiliateWP history synced from WordPress' },

  { id: 'orders', label: 'Commissions (orders)', hint: 'BB orders with affiliate coupon — ledger by order date' },
...
function rowKey(r, wpView) {

  if (wpView) return String(r.referral_id)
...
export default function Referrals() {
  ...
  const view = searchParams.get('view') === 'orders' ? 'orders' : 'wp'

  const wpView = view === 'wp'
  ...
  const bbFilter = searchParams.get('bb') || ''
  ...
  const statusTabs = wpView ? WP_STATUSES : ORDER_STATUSES
  ...
  const setView = (next) => {
    ...
    if (next === 'wp') p.delete('view')

    else p.set('view', 'orders')

    const st = p.get('status') || ''

    if (next === 'wp' && (st === 'estimated' || st === 'open')) p.delete('status')

    if (next === 'orders' && st && !ORDER_STATUSES.includes(st)) p.delete('status')
  ...
  const load = useCallback(() => {
    ...
    if (wpView && couponFilter) params.coupon = couponFilter

    if (wpView && bbFilter) params.bb = bbFilter
  ...
  }, [wpView, status, search, affId, couponFilter, bbFilter, dateFrom, dateTo, monthFilter, offset])
  ...
  const toggleAll = () =>
    ...
    setSelected(selected.size === referrals.length ? new Set() : new Set(referrals.map(r => rowKey(r, wpView))))
  ...
  const bulkUpdate = async (newStatus) => {
    ...
    if (!confirm(`Change ${selected.size} referral rows to "${newStatus}"?`)) return
  ...
  const updateOne = async (r, newStatus) => {
    ...
    try {
      ...
      if (wpView) {
        ...
        await api.updateReferral(String(r.referral_id), { status: newStatus })
      ...
      setReferrals(prev => prev.map(x => rowKey(x, wpView) === rowKey(r, wpView) ? { ...x, status: newStatus } : x))
  ...
  const exportColumns = wpView

    ? [

        { header: 'Referral ID', value: r => r.referral_id },
        ...
        { header: 'Order', value: r => r.salesorder_number },
        ...
        { header: 'In ledger', value: r => r.in_ledger ? 'yes' : 'no' },
  ...
  const tableHeaders = wpView

    ? ['Referral', 'Date', 'Affiliate', 'Coupon', 'WC #', 'Order', 'Commission', 'Status', 'Ledger', 'Actions']

    : ['Order', 'Date', 'Affiliate', 'Coupon', 'WC #', 'Commission', 'Status']
    ...
    <div>
      ...
      <PageHeader
        ...
        subtitle={wpView
          ...
          ? `${total.toLocaleString()} AffiliateWP referrals · WP referral date`

          : `${total} orders with affiliate coupons · by order date (Supabase)`}

        actions={

          <ExportButtons

            baseName={wpView ? 'wp-referrals' : 'commissions'}

            sheetName={wpView ? 'WP Referrals' : 'Commissions'}
      ...
      <p className="px-6 -mt-2 mb-4 text-xs text-gray-500">
        ...
        {wpView ? (
          ...
          <>
            ...
            Full history synced from WordPress/AffiliateWP. <strong>In ledger</strong> means the referral is linked to an order in Commissions.

            Status changes sync back to AffiliateWP.
          ...
          <>
            ...
            Use <strong>All WP referrals</strong> to mark commissions paid in AffiliateWP.
      ...
      {wpView && (
        <div className="flex flex-wrap items-center gap-3 px-6 mb-4">
          <select className="select w-44" value={couponFilter} onChange={e => set('coupon', e.target.value)}>
            <option value="">Coupon: all</option>
            <option value="yes">With coupon</option>
            <option value="no">Without coupon</option>
          </select>
          <select className="select w-44" value={bbFilter} onChange={e => set('bb', e.target.value)}>
            <option value="">Order: all</option>
            <option value="yes">BB orders</option>
            <option value="no">Non-BB</option>
          </select>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 px-6 mb-4">
        ...
        <div className="relative w-56">
          ...
          <input
            ...
            placeholder={wpView ? 'Referral, WC #, affiliate…' : 'Order, coupon, affiliate…'}
        ...
        {(search || affId || dateFrom || dateTo || monthFilter || couponFilter || bbFilter) && (
          ...
          <table className="w-full">
            ...
            <thead>
              ...
              <tr className="border-b">
                ...
                {tableHeaders.map(h => <th key={h} className="th">{h}</th>)}
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {loading
                ...
                ? <tr><td colSpan={tableHeaders.length + (wpView ? 1 : 0)}><Spinner /></td></tr>

                : referrals.length === 0

                  ? <tr><td colSpan={tableHeaders.length + (wpView ? 1 : 0)}><Empty label="No rows for these filters" /></td></tr>

                  : referrals.map(r => {

                    const id = rowKey(r, wpView)
                      ...
                      <tr key={id} className={`tr-hover ${selected.has(id) ? 'bg-blue-50' : ''}`}>
                        ...
                        {wpView ? (
                          ...
                          <>
                            ...
                            <td className="td font-mono text-xs">{r.salesorder_number || '—'}</td>
